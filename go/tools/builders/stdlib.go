...
You may need to use the flags --cpu=x64_windows --compiler=mingw-gcc.`)
	...
	originalOS := os.Getenv("GOOS")
	originalARCH := os.Getenv("GOARCH")
	os.Setenv("GOOS", runtime.GOOS)
	os.Setenv("GOARCH", runtime.GOARCH)
	// toolArgs := goenv.goCmd("build", "-C", "external/go_sdk/src", "-pkgdir", "external/go_sdk", "cmd/pack")
	...
	if err := simpleMoveFile("./pack", filepath.Join(output,"pkg/tool", runtime.GOOS + "_" + runtime.GOARCH, "pack")); err != nil {
		return err
	}
...
// simpleMoveFile demonstrates moving a file using os.Rename.
// This is efficient but typically only works when src and dst are on the same volume.
func simpleMoveFile(src, dst string) error {
