...
func instrumentForCoverage(goenv *env, importPath string, pkgName string, infiles []string, srcNames []string, coverVar string, coverMode string, outfiles []string, workDir string) ([]string, error) {
	...
	// write pkgcfg and covoutputs files
	pcfg := coverPkgConfig{
		PkgPath:   importPath,
		PkgName:   pkgName,
	...
	// ROOTCAUSE: is that srcName which is adjusted is not used in the profile
	// register is not needed in the new implementation
	return outfiles, nil
}
...
// copied from https://cs.opensource.google/go/go/+/refs/tags/go1.24.4:src/cmd/internal/cov/covcmd/cmddefs.go;l=18
