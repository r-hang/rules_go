...
func (fp *FlatPackage) FilterCgoSourceFiles(prf PathResolverFunc) error {
	...
	if fp.CgoGenerated != "" {
		...
		if _, err := os.Stat(filepath.Join("/home/user/go-code", fp.CgoGenerated, "_cgo_gotypes.go")); err != nil && os.IsNotExist(err) {
			// NOTE: this skipping happens because cgo processed code is not in the sandbox as it was not generated by bazel
			return nil
		}
...
func (fp *FlatPackage) ResolveImports(resolve ResolvePkgFunc, overlays map[string][]byte) error {
	...
	// Imports are missing because cgo preprocessed code is not in the sandbox
	return nil
}
